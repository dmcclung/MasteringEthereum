...
contract AuctionRepository {
     ...
     * @param _auctionId uint ID of the auction to validate its ownership to msg.sender
     */
    modifier isOwner(uint _auctionId) {
        require(auctions[_auctionId].owner == msg.sender, "Is not owner");
        _;
    }
    ...
    modifier contractIsDeedOwner(address _deedRepositoryAddress, uint256 _deedId) {
        ...
        require(deedOwner == address(this), "Contract is not deed owner");
        _;
    }
    ...
    /**
     * @dev Gets the number of auctions
     ...
     * @param bool whether the auction is created
     */
    function createAuction(
        ...
        string _metadata,
        uint256 _startPrice,
        uint _blockDeadline) public returns(bool) {
    ...
    /**
     * @dev Finalize a closed auction
     * @dev The auction should be closed, and there should be at least one bid
     ...
     * @dev Auction should be active and not closed
     * @dev Refund previous bidder if a new bid is valid
     * @param _auctionId uint ID of the created auction
     */
    ...
    // AuctionCreated is fired when an auction is canceled
    event AuctionCanceled(address _owner, uint auctionId);
